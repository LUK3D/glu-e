...
export default function OutlinerItem({onAddColumnClick, onTableClick, table}:{table:ITable, onTableClick:Function, onAddColumnClick:Function}) {
    ...
    <li className="pl-10 pr-4 py-2 flex flex-col group">
        ...
        <ul className={`pl-4 ${table.expanded == true?' flex flex-col':'hidden'}`}>
            {table.columns.map((column)=>(
                <li key={generateUniqueKey()} className="pl-1  py-2 flex flex-col border-l border-black-300" onDragStart={(event) => onDragStart(event, 'table', column)} draggable>
